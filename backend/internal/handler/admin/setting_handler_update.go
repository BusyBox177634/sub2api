...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.QuickOpsMonitorSuffix != nil {
		quickOpsMonitorSuffix = strings.TrimSpace(*req.QuickOpsMonitorSuffix)
	}
	if quickOpsMonitorEnabled {
		if _, ok := service.ValidateQuickOpsMonitorSuffix(quickOpsMonitorSuffix); !ok {
			response.BadRequest(c, "Quick Ops Monitor suffix must be 4-64 characters and contain only letters, numbers, hyphen, or underscore")
			return
		}
	} else {
		quickOpsMonitorSuffix = service.NormalizeQuickOpsMonitorSuffix(quickOpsMonitorSuffix)
	}
	...
	settings := &service.SystemSettings{
		...
		UsageBriefEnabled: func() bool {
			if req.UsageBriefEnabled != nil {
				return *req.UsageBriefEnabled
			}
			return previousSettings.UsageBriefEnabled
		}(),
		QuickOpsMonitorEnabled: quickOpsMonitorEnabled,
		QuickOpsMonitorSuffix:  quickOpsMonitorSuffix,
