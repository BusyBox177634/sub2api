...
func TestUpdateSettingsQuickOpsMonitorRequiresSuffixWhenEnabled(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{})

	rec := doUpdateSettings(t, h, map[string]any{"quick_ops_monitor_enabled": true}, nil)

	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.NotEqual(t, "true", repo.values[service.SettingKeyQuickOpsMonitorEnabled])
}

func TestUpdateSettingsQuickOpsMonitorPersistsValidSuffix(t *testing.T) {
	h, repo := newStepUpSwitchTestHandler(t, map[string]string{})

	rec := doUpdateSettings(t, h, map[string]any{
		"quick_ops_monitor_enabled": true,
		"quick_ops_monitor_suffix":  "/opsview/",
	}, nil)

	require.Equal(t, http.StatusOK, rec.Code)
	require.Equal(t, "true", repo.values[service.SettingKeyQuickOpsMonitorEnabled])
	require.Equal(t, "opsview", repo.values[service.SettingKeyQuickOpsMonitorSuffix])
}
