...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		result, usageDetailCapture, err := captureOpenAIForwardUsageDetail(c, body, false, func() (*service.OpenAIForwardResult, error) {
			defer func() {
		...
		if endpoint == service.GrokMediaEndpointVideoStatus || endpoint == service.GrokMediaEndpointVideoContent {
			...
			if billResult := prepareGrokVideoCompletionBilling(requestCtx, h, reqLog, apiKey, subject, taskID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, account, billResult, billResult.Model, body, taskID, usageDetailCapture)
			}
		} else if shouldRecordGrokMediaUsage(endpoint, requestModel, result) {
			recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, account, result, requestModel, body, requestID, usageDetailCapture)
		}
...
func recordGrokMediaUsage(
	...
	body []byte,
	requestID string,
	usageDetailCapture *service.UsageLogDetailCapture,
) {
