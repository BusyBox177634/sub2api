...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			result, usageDetailCapture, err := captureForwardUsageDetail(c, body, reqStream, func() (*service.ForwardResult, error) {
				...
				return h.geminiCompatService.Forward(requestCtx, c, account, body)
			})
	...
	for {
		...
		for {
			...
			// 记录 Forward 前已写入字节数，Forward 后若增加则说明 SSE 内容已发，禁止 failover
			writerSizeBeforeForward := c.Writer.Size()
			result, usageDetailCapture, err := captureForwardUsageDetail(c, attemptBody, reqStream, func() (*service.ForwardResult, error) {
				if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
					return h.antigravityGatewayService.Forward(requestCtx, c, account, attemptBody, hasBoundSession)
				}
