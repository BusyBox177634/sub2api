...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	if result := grokRealtimeBillingResult(model, elapsed, audioObserved); result != nil {
		h.recordGrokVoiceUsage(c, apiKey, selection.Account, subscription, "realtime", nil, result, nil)
	}
}
...
func (h *OpenAIGatewayHandler) GrokVoice(c *gin.Context, endpoint string) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		result, usageDetailCapture, forwardErr := captureOpenAIForwardUsageDetail(c, body, false, func() (*service.OpenAIForwardResult, error) {
			defer release()
			return h.gatewayService.ForwardGrokVoice(c.Request.Context(), c, account, endpoint, body, contentType)
		})
		if forwardErr == nil {
			h.recordGrokVoiceUsage(c, apiKey, account, subscription, endpoint, body, result, usageDetailCapture)
			return
		}
