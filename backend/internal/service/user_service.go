...
type UserUpdateFields struct {
	...
	LastLoginAt  bool
	LastActiveAt bool
	// UsageBriefPreferences 覆盖 usage_brief_auto_email_enabled /
	// usage_brief_page_enabled。两项由同一次资料更新共同维护，避免页面关闭后
	// 自动邮件偏好被并发的陈旧快照重新打开。
	UsageBriefPreferences bool
...
// UpdateUsernameFromTrustedIdentity syncs a username chosen during a trusted
// registration/identity flow. It is intentionally separate from UpdateProfile,
// where user-submitted username changes are rejected.
func (s *UserService) UpdateUsernameFromTrustedIdentity(ctx context.Context, userID int64, username string) (*User, error) {
...
func (s *UserService) updateProfile(ctx context.Context, userID int64, req UpdateProfileRequest) (*User, int, error) {
	...
	if req.UsageBriefAutoEmailEnabled != nil {
		...
		fields.UsageBriefPreferences = true
	}
