...
func TestOpenAIGatewayServiceRecordUsage_PersistsUsageDetailCaptureWhenProvided(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "openai_detail_capture",
			Usage: OpenAIUsage{
				InputTokens:  10,
				OutputTokens: 6,
			},
		...
		UsageDetailCapture: &UsageLogDetailCapture{
			RequestBody:       []byte(`{"model":"gpt-5.1","messages":[{"role":"user","content":"hello detail"}]}`),
			ResponseBody:      []byte(`{"id":"chatcmpl_detail","choices":[{"message":{"role":"assistant","content":"world detail"}}]}`),
			ResponseBodyBytes: len(`{"id":"chatcmpl_detail","choices":[{"message":{"role":"assistant","content":"world detail"}}]}`),
	...
	require.Equal(t, "openai_detail_capture", detailRepo.lastRequestID)
	...
	require.Contains(t, *detailRepo.lastDetail.RequestPayloadJSON, "hello detail")
	require.NotNil(t, detailRepo.lastDetail.ResponsePayloadJSON)
	require.Contains(t, *detailRepo.lastDetail.ResponsePayloadJSON, "world detail")
	require.NotNil(t, detailRepo.lastDetail.CompressedRequestPayloadJSON)
	require.JSONEq(t, `{"messages":[{"role":"user","content":"hello detail"}]}`, *detailRepo.lastDetail.CompressedRequestPayloadJSON)
	require.NotNil(t, detailRepo.lastDetail.CompressedResponsePayloadJSON)
	require.JSONEq(t, `{"messages":[{"role":"assistant","content":"world detail"}]}`, *detailRepo.lastDetail.CompressedResponsePayloadJSON)
}
...
func TestOpenAIGatewayServiceRecordUsage_BillingErrorWritesUnsettledUsageLog(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		...
		UsageDetailCapture: &UsageLogDetailCapture{
			RequestBody:       []byte(`{"input":"billing failure request"}`),
			ResponseBody:      []byte(`{"output":"billing failure response"}`),
			ResponseBodyBytes: len(`{"output":"billing failure response"}`),
			ResponseFormat:    UsageLogDetailResponseFormatJSON,
		},
	})
	...
	require.Equal(t, 1, detailRepo.upsertCalls)
	require.NotNil(t, detailRepo.lastDetail)
	require.Contains(t, *detailRepo.lastDetail.RequestPayloadJSON, "billing failure request")
}
