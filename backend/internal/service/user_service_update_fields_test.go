...
func TestUpdateProfile_OnlyDeclaresRequestedColumns(t *testing.T) {
	tests := []struct {
		...
		{
			name: "usage brief auto email only",
			req:  UpdateProfileRequest{UsageBriefAutoEmailEnabled: boolPtr(true)},
			want: UserUpdateFields{UsageBriefPreferences: true},
		},
		{
		...
		{
			name: "usage brief page and notify threshold",
			req: UpdateProfileRequest{
				UsageBriefPageEnabled:  boolPtr(false),
				BalanceNotifyThreshold: float64Ptr(1.5),
			},
			want: UserUpdateFields{UsageBriefPreferences: true, BalanceNotifySettings: true},
		},
	}
