...
func TestGatewayServiceRecordUsage_BillingErrorWritesUnsettledUsageLog(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &RecordUsageInput{
		...
		UsageDetailCapture: &UsageLogDetailCapture{
			RequestBody:       []byte(`{"messages":[{"role":"user","content":"billing failure request"}]}`),
			ResponseBody:      []byte(`{"content":"billing failure response"}`),
			ResponseBodyBytes: len(`{"content":"billing failure response"}`),
			ResponseFormat:    UsageLogDetailResponseFormatJSON,
		},
	})
