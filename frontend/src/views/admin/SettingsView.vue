    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.usageBrief.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.usageBrief.description') }}
            </p>
            <p class="mt-1.5 text-xs">
              <router-link
                to="/admin/usage-brief"
                class="inline-flex items-center gap-1 text-primary-600 hover:underline dark:text-primary-400"
              >
                {{ t('admin.settings.features.usageBrief.configureLink') }}
                <span aria-hidden="true">→</span>
              </router-link>
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.usageBrief.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.usageBrief.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.usage_brief_enabled" />
            </div>
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            ...
            <div v-if="form.quick_ops_monitor_enabled">
              <label class="input-label">
        ...
        <div class="card">
          ...
          <div class="space-y-5 p-6">
            ...
            <div v-if="form.model_plaza_enabled">
              ...
              <textarea v-model="form.model_plaza_description" rows="6" class="input font-mono text-sm"></textarea>
            </div>
          </div>
        </div>
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      quick_ops_monitor_suffix: form.quick_ops_monitor_suffix.trim(),
      // Model Plaza feature switches + description
