...
import type { UsageLog, ApiKey, UsageLogDetailResponse, UsageQueryParams, UsageStatsResponse, UserErrorRequest } from '@/types'
...
const openDetail = async (row: UsageLog) => {
  detailRecord.value = row
  detailData.value = null
  detailDialogVisible.value = true
  detailLoading.value = true
  try {
    detailData.value = await usageAPI.getDetail(row.id)
  } catch (error) {
    console.error('[UsageView] load detail failed:', error)
