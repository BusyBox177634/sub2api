...
describe('user UsageView tooltip', () => {
  ...
  it('closes the detail dialog when detail loading fails', async () => {
    ...
    await flushPromises()
    const setupState = (wrapper.vm as any).$?.setupState
    await setupState.openDetail({ id: 100 })
    await flushPromises()

    expect(getDetail).toHaveBeenCalledWith(100)
  ...
  it('keeps historical unavailable detail open when the API returns available false', async () => {
    query.mockResolvedValue({
      items: [],
      total: 0,
      pages: 0,
    ...
    await flushPromises()
    const setupState = (wrapper.vm as any).$?.setupState
    await setupState.openDetail({ id: 101 })
    await flushPromises()

    expect(getDetail).toHaveBeenCalledWith(101)
    expect(showError).not.toHaveBeenCalled()
    expect(setupState.detailDialogVisible).toBe(true)
    expect(setupState.detailData.available).toBe(false)
    expect(setupState.detailData.reason).toBe('historical')
  })
