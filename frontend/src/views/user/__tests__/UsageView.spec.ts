...
const messages: Record<string, string> = {
  ...
  'usage.ipGeo.batchFailed': 'IP lookup failed',
  'usage.errors.allKeys': 'All keys',
  'usage.errors.model': 'Model',
  'usage.errors.modelPlaceholder': 'Model',
...
const UsageTableStub = {
  props: ['data'],
  emits: ['detailClick', 'sort', 'ipGeoBatchFailed'],
  template: `
    <div>
      <button
        v-for="row in data"
...
const UsageLogDetailDialogStub = {
  props: ['show', 'loading', 'detail', 'record'],
  emits: ['close'],
  template: '<div v-if="show" data-testid="detail-dialog">{{ loading ? "loading" : (detail?.reason || record?.request_id) }}</div>',
}
  ...
  return mount(UsageView, {
    global: {
      stubs: {
        ...
        UsageLogDetailDialog: UsageLogDetailDialogStub,
      },
    },
  })
}
...
describe('user UsageView', () => {
  ...
  it('opens the usage detail dialog from the table detail event', async () => {
    const wrapper = mountUsageView()
    await flushPromises()

    await wrapper.get('[data-testid="detail-button"]').trigger('click')
    await flushPromises()

    expect(getDetail).toHaveBeenCalledWith(usageLog.id)
    expect(wrapper.get('[data-testid="detail-dialog"]').text()).toContain('historical')
    expect(showError).not.toHaveBeenCalled()
  })

  it('closes the detail dialog and shows an error when detail loading fails', async () => {
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
    getDetail.mockRejectedValueOnce(new Error('detail failed'))
    const wrapper = mountUsageView()
    await flushPromises()
