export default {
  ...
  quickMonitor: {
    ...
    readOnly: '只读视图',
    suffix: '访问后缀',
    searchByUser: '搜索用户名或邮箱',
    noFilteredUsers: '当前筛选条件下暂无用户',
    noFilteredSubscriptions: '当前筛选条件下暂无订阅',
    recordsUnit: '条记录',
    reportsUnit: '{count} 份报告',
    ...
    ops: {
      ...
      noSystemSnapshot: '暂无系统资源快照',
      usageDetailRetention: '使用记录详情清理',
      noRetentionStatus: '暂无清理状态',
      diskMountDetails: '磁盘挂载点明细',
      diskMountHint: '应用进程可见且可信的挂载点',
      noDiskMounts: '暂无磁盘挂载点数据',
      status: {
        noData: '暂无数据',
        stopped: '已停止',
        running: '清理中',
      ...
      metrics: {
        ...
        successError: '成功 {success} / 错误 {error}',
        upstreamSla: '上游 {upstream} · SLA {sla}',
        latencyHint: 'P95 {p95} ms · 平均 {avg} ms',
        tpsPeak: 'TPS 峰值 {value}',
        diskFree: '{mount} · {free} 可用',
        cleanupHint: '待处理 {pending} · 失败 {failed}',
        queueDepth: '队列 {value}',
        accountSwitch: '账号切换',
      },
      retention: {
        ...
        processed: '已处理',
        cleaned: '已清理',
        compressedRequest: '压缩请求',
        compressedResponse: '压缩响应',
        fallbackSkipped: '降级/跳过',
        failed: '失败',
        startedAt: '最近开始',
        finishedAt: '最近结束',
        nextRun: '下次运行',
  ...
  usage: {
    ...
    compressedRequestJson: '压缩请求 JSON',
    compressedResponseJson: '压缩响应 JSON',
    copyJson: '复制 JSON',
    jsonCopied: 'JSON 已复制',
    fullJsonCleaned: '完整 JSON 已按保留策略清理，请查看压缩 JSON。',
    detailLoadFailed: '加载请求详情失败',
    unavailable: '详情不可用',
    unavailableDisabled: '请求详情记录未启用。',
    unavailableHistorical: '这是历史记录，当时尚未记录请求详情。',
    unavailableNotCaptured: '这条记录没有捕获到可展示的请求或响应内容。',
    sentMessages: '发送内容',
    receivedMessages: '返回内容',
    noMessages: '暂无可展示内容',
  ...
  usageBrief: {
    title: '用量简报',
    description: '查看基于个人 API 使用记录生成的日报、周报和月报',
    adminDescription: '管理所有普通用户的用量简报、生成队列和 AI 配置',
  },
  ...
  admin: {
    ...
    ops: {
      ...
      mountPoint: '挂载点',
      diskRole: '用途',
      diskRoleDataDir: '数据目录',
      diskRoleContainerRoot: '容器根文件系统',
      diskRoleMount: '挂载点',
      device: '设备',
      filesystem: '文件系统',
      used: '已用',
      ...
      usageDetailRetention: '详情清理',
      retentionRunning: '清理中',
      retentionWaiting: '待继续',
      retentionIdle: '空闲',
      retentionStopped: '已停止',
      remaining: '待清理',
      processed: '已处理',
      cleaned: '已清理',
      compressed: '已压缩',
      ...
      tooltips: {
        ...
        disk: '应用数据目录优先的磁盘容量；明细中展示应用进程可见挂载点的总量、已用、可用和使用率。',
        db: '数据库连接池状态，包括活跃连接、空闲连接和等待连接数。',
        redis: 'Redis 连接池状态，显示活跃和空闲的连接数。',
        jobs: '后台任务执行状态，包括最近运行时间、成功时间和错误信息。',
        usageDetailRetention: '使用记录详情完整版 JSON 的后台清理状态。当前只清理昨天 00:00 到今天 00:00 的完整请求/响应 JSON，清理前会保留压缩版。',
    ...
    settings: {
      ...
      features: {
        ...
        usageBrief: {
          title: '用量简报',
          description: '启用后管理员和普通用户侧边栏显示用量简报。后台会为所有普通用户生成日报、周报和月报，管理员可查看生成队列并配置 AI 生成接口。',
          configureLink: '前往 用量简报 配置 AI 接口和生成队列',
          enabled: '启用用量简报',
          enabledHint: '关闭后侧边栏入口隐藏，自动生成调度停止；已有报告和队列数据保留。',
        },
        quickOpsMonitor: {
          title: '运维快捷监控',
          description: '开启后可通过配置的单段后缀访问无需登录的只读后台页面。请使用随机、足够长的后缀。',
          enabled: '启用运维快捷监控',
          enabledHint: '入口仅提供查看能力，不开放新增、编辑、删除、禁用、清理或触发生成等操作。',
          suffix: '访问后缀',
          suffixPlaceholder: '例如 ABCD1234',
          suffixHint: '4-64 位，只允许字母、数字、短横线和下划线；不能使用 admin、api 等已有路由。',
        },
        riskControl: {
          title: '风控中心',
