export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，收敛会使用账号级持久随机身份。仅设备档只收敛设备；设备+会话档保留按客户端隔离的匿名会话，并同步 body 缓存键；完全收敛档使用账号级统一会话。默认关闭（原样透传客户端标识）；部分账号开启收敛后出现过额度缩水，请按自己的实测结果选择。',
