export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'When multiple users share an OAuth account, convergence uses a durable per-account random identity. Device mode converges only the device; Session keeps separate pseudonymous client sessions and aligns the body cache key; Full uses one account-level session. Off by default (client identifiers pass through as-is). Some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
